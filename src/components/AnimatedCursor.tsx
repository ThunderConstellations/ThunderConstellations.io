...
const AnimatedCursor = () => {
  ...
  const EnergyOrb = ({ delay, distance, size, opacity }: {
    ...
    <div
      className={`absolute rounded-full transition-all duration-150 ${isMoving ? `opacity-${Math.floor(opacity * 100)}` : 'opacity-20'
  ...
  const StarParticle = ({ delay, radius, size }: { delay: number; radius: number; size: number }) => (
    <div
      className={`absolute transition-all duration-200 ${isMoving ? 'opacity-70' : 'opacity-30'
        }`}
      style={{
  ...
  const ElectricTrail = ({ delay, distance, length }: { delay: number; distance: number; length: number }) => (
    <div
      className={`absolute transition-all duration-100 ${isMoving ? 'opacity-60' : 'opacity-0'
        }`}
      style={{
  ...
  return (
    <div className="pointer-events-none fixed inset-0 z-50">
      ...
      <div
        className={`absolute transition-all duration-75 ease-out ${isMoving ? 'opacity-100 scale-100' : 'opacity-60 scale-75'
      ...
      {/* Inner Energy Ring */}
      <div
        className={`absolute rounded-full transition-all duration-200 ${isMoving
            ? 'opacity-50'
            : 'opacity-20'
          } ${isClicking ? 'scale-150' : 'scale-100'}`}
        style={{
      ...
      {/* Outer Energy Ring */}
      <div
        className={`absolute rounded-full border transition-all duration-200 ${isMoving
      ...
      <div
        className={`absolute rounded-full transition-all duration-300 ${isMoving
            ? 'opacity-30'
            : 'opacity-10'
